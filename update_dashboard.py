...
def download_excel(token):
    ...
    if not download_url:
        raise Exception('downloadUrl nao encontrado')
...
def inject_into_html(xlsx_bytes):
    ...
    # Injetar o xlsx como base64 no IRAW via localStorage simulado
    # Substituir o script de init para auto-carregar os dados
    auto_script = (
        '<script>\n'
        'window._autoXlsxB64 = "' + b64 + '";\n'
        '</script>\n'
    )

    # Inserir logo apos o script do xlsx.js
    xlsx_script_tag = '<script src="https://cdnjs.cloudflare.com/ajax/libs/xlsx/0.18.5/xlsx.full.min.js"></script>'
    html = html.replace(
        xlsx_script_tag,
        xlsx_script_tag + '\n' + auto_script
    )

    # Adicionar script de auto-parse no DOMContentLoaded
    auto_parse = (
        '\n<script>\n'
        'window.addEventListener("DOMContentLoaded", function() {\n'
        '  if (!window._autoXlsxB64) return;\n'
        '  try {\n'
        '    var bin = atob(window._autoXlsxB64);\n'
    ...
    html = html.replace('</body>', auto_parse + '</body>')
